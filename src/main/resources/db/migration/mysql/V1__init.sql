-- V1__init.sql (MySQL용 전체)

-- 1. 기초 테이블 생성 (외래키 참조 대상 먼저)
...
(
    ...
    constraint fk_notice_board foreign key (board_id) references abstract_board (board_id) on delete cascade
);
...
(
    ...
    constraint UK7blitxweiw758g32lg3bhavf6 unique (email, provider),
...
(
    ...
    board_id  bigint null,
    member_id bigint null,
    constraint UKfwpcqlf7jjnlg0xags8noqoc7 unique (board_id, member_id),
    constraint board_id unique (board_id, member_id),
...
(
    ...
    created_date       datetime     null,
    last_modified_date datetime     null,
    constraint fk_notice foreign key (notice_id) references notice (notice_id) on delete cascade
);
...
(
    ...
    shop_id          bigint       null,
    constraint UK3rs8jisxaeew2dfx9m3qvx6fl unique (member_id, order_id),
